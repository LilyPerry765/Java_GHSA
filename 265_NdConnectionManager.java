...
public class NdConnectionManager extends BaseConnection{
  ...
  {
    logger.log(Level.FINE, "Cavisson-Plugin|testNDConnection() method is  called. rest url -"+ restUrl);
    if(checkAndMakeConnection(URLConnectionString, restUrl, errMsg, test, consoleLogger))
    {
      logger.log(Level.FINE, "Cavisson-Plugin|After check connection method.");

//      if((getResult() == null))
//      {
//	logger.log(Level.FINE, "result is null- ",err);
...
//      }
    ...
    {
      logger.log(Level.FINE, "Cavisson-Plugin|connection is fail - ",err);
      errMsg.append(err);
      return false;
    }
  ...
  {
    logger.log(Level.FINE, "Cavisson-Plugin|checkAndMakeConnection method called. with arguments restUrl : ", new Object[]{restUrl});
    try
    {
       restUrl = getUrlString();
      if(ndParam != null)
      {
	logger.log(Level.FINE, "Cavisson-Plugin|checkAndMakeConnection method called. with ndParam "+  ndParam.toString());
	...
	url = new URL((restUrl+"/ProductUI/productSummary/jenkinsService/reportData?reportParam="+test+"&status=true").replaceAll(" ","%20")); //for only test connection
      ...
      logger.log(Level.FINE, "Cavisson-Plugin|checkAndMakeConnection method called. with arguments url"+  url);
...
//        err = (String)resonseObj.get("errMsg");
  ...
  {
    logger.log(Level.FINE, "Cavisson-Plugin|fetchMetricData() called. get resulet"+ getResult());
    JSONObject resonseObj = null;
    resonseObj =  (JSONObject) JSONSerializer.toJSON(this.result);
    if(resonseObj == null)
    {
      logger.log(Level.SEVERE, "Cavisson-Plugin|Not able to get response form server due to: ");
      return null;
    }
  ...
  private MetricDataContainer parseJSONData(JSONObject resonseObj, String testMode)
  {
    ...
    logger.log(Level.FINE,"Cavisson-Plugin|Recived response from : " + resonseObj );
 
    try{
      ArrayList<MetricData> dataList = new ArrayList<MetricData>();
      JSONObject jsonGraphs = (JSONObject)resonseObj.get("graphs");
      logger.log(Level.FINE,"Cavisson-Plugin|Recived response from graphs : " + jsonGraphs );
      int freq = ((Integer)resonseObj.get("frequency"))/1000; 
      logger.log(Level.FINE,"Cavisson-Plugin|Recived response from : frq = " + freq );
	...
	metricDataContainer.setCustomHTMLReport((String)resonseObj.get("customHTMLReport"));
      ...
      {
	logger.log(Level.FINE,"Cavisson-Plugin|Recived response inside test mode : "  );
	testReport = new TestReport();
	JSONObject jsonTestReportWholeObj = resonseObj.getJSONObject("testReport");
	logger.log(Level.FINE,"Cavisson-Plugin|Recived response from whole report : " + jsonTestReportWholeObj );
	JSONObject jsonTestReport = jsonTestReportWholeObj.getJSONObject("members");

	logger.log(Level.FINE,"Cavisson-Plugin|Recived response from test report : " + jsonTestReport );
...
//          reportLink = prefix[0]+reportLink;
   ...
   public String getUrlString()
  {
   ...
   {
 	 try {
 		 logger.log(Level.FINE, "Cavisson-Plugin|getting profile list ");
          ...
          String str = getUrlString(); // URLConnectionString.substring(0,URLConnectionString.lastIndexOf("/"));
 	      ...
 	      if(((HttpURLConnection) conn).getResponseCode()!= 200) {
 	    	  logger.log(Level.FINE, "Cavisson-Plugin|getting error in fetching profile list.");
 	      ...
 	      logger.log(Level.FINE, "Cavisson-Plugin|profiles- " + profiles.size());
 	      return profiles;
 	 }
